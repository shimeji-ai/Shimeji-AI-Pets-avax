document.addEventListener("DOMContentLoaded", () => {
  ...
  function storageSessionSet(obj) {
    ...
    return new Promise((resolve) => {
      chrome.storage.local.set(prefixed, () => resolve(true));
    });
  }
  ...
  function storageSessionRemove(keys) {
    ...
    return new Promise((resolve) => {
      chrome.storage.local.remove(prefixed, () => resolve(true));
    });
  }
...
function buildShimejiPreview(shimeji) {
  ...
  async function getSessionMasterKey() {
    return new Promise((resolve) => {
      chrome.storage.local.get([SESSION_PREFIX + 'masterKey'], (data) => {
        resolve(data[SESSION_PREFIX + 'masterKey'] || '');
      });
    });
  }

  function setSessionMasterKey(value) {
    chrome.storage.local.set({ [SESSION_PREFIX + 'masterKey']: value });
  }

  function clearSessionMasterKey() {
    chrome.storage.local.remove([SESSION_PREFIX + 'masterKey']);
  }
