  ...
  for (const token of owned) {
    ...
    const shimejiProps =
      metadata?.properties && typeof metadata.properties === "object" && !Array.isArray(metadata.properties)
        ? (metadata.properties as Record<string, unknown>).shimeji
        : null;
    const spritesBaseUri =
      shimejiProps && typeof shimejiProps === "object" && !Array.isArray(shimejiProps)
        ? resolveMediaUrl(String((shimejiProps as Record<string, unknown>).spritesBaseUri || ""))
        : null;
    const explicitName = String(metadata?.name || "").trim();
    ...
    byCharacter.set(characterId, {
      id: characterId,
      name: explicitName || titleCaseCharacterId(characterId),
      tokenCount: 1,
