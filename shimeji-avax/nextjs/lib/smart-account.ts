// Shimeji Passport — Smart Account via ZeroDev Kernel v3 + WebAuthn Passkeys
//
...
//               scoped permissions) so the Shimeji pet can interact with
...
//      only allowed to call Shimeji contracts, valid for N days.
...
//   sponsors gas so users pay nothing for Shimeji interactions.

import { createPublicClient, http } from "viem";
import { ACTIVE_CHAIN, RPC_URL, SHIMEJI_NETWORK } from "@/lib/contracts";
...
export const SMART_ACCOUNT_AVAILABLE =
  Boolean(PROJECT_ID) && SHIMEJI_NETWORK !== "local";
...
export function getSmartAccountSupport(): SmartAccountSupport {
  if (!PROJECT_ID) {
    return {
      supported: false,
      reason: "Shimeji Passport is not configured for this environment yet.",
    };
  }

  if (SHIMEJI_NETWORK === "local") {
    return {
      supported: false,
      reason: "Shimeji Passport is disabled on the local network.",
    };
  }
  ...
  if (typeof window === "undefined") {
    return {
      supported: false,
      reason: "Shimeji Passport is only available in the browser.",
    };
  }
...
export async function createSmartAccountFromPasskey(
  ...
  // are gas-sponsored (users pay 0 AVAX gas for Shimeji interactions).
